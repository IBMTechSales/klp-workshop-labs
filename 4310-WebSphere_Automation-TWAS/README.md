...
Duration: 
  ...
  - CVE protection: 60 minutes
  - Memory leak investigation: 45 minutes
...
1. Clone the GitHub repo to download the lab artifacts and Linux shell scripts that are used in the lab. Then, set execute permissions for the scripts. 
...
1. Log in to Red Hat OpenShift CLI:
    ...
    b. Run the following command to login to OpenShift
...
2.  Go to the `websphere-automation`project in OpenShift
...
3. Run the following command to verify that IBM WebSphere Automation `ready` in your environment. 
...
1.  Before you start to register servers with IBM WebSphere Automation, configure an email to receive notifications about CVE's that impact registred servers. 
...
2.  The Email server configuration is pre-configured for this lab. You only need to add your personal email to receive notifications of new security vulnerabilities.
...
3. Return to the `Security` view in the WebSphere Automation dashboard

    ![notification config](./lab1-media/media/image16b.png)
...
1.  Minimize the browser window and return to the `terminal` window, or open a new terminal window if you closed the one used previosuly. 
...
## Section 5 - Memory leak investigation with IBM WebSphere Automation and Instana
...
However, for your reference, the detailed steps for configuring Instana alerts for WebSphere Application Server is documented in **`Appendix A`** of this lab guide. **DO NOT EXECUTE THE STEPS IN APPENDIX A**
...
2.  Login to WebSphere Automation, if you are not already logged in
    ...
    <a href="hhttps://cpd-websphere-automation.apps.demo.ibmdte.net/zen/#/homepage"><span class="underline">https://cpd-websphere-automation.apps.demo.ibmdte.net/zen/#/homepage</span></a>
    ...
    **Note:** If necessary, accept all the browser security warnings and certificates. In Firefox, click **Advanced**, and then scroll down and click **Accept the risk and continue**. 
    ...
    c. Select **`htpasswd`** as the login selection
  
    ![ocp ui login](./lab1-media/media/image12.png)
	
		
    d.  Enter **`ibmadmin`** as username and **`engageibm`** as password. And click **`Log In`**.
...
7.  From the Terminal window, run the provided script which will run a sample memory leak application in WebSphere. Instana will immediately show the significant increase in JVM heap usage, which will create an application memory leak event.
    ...
    <tbody>
    <tr class="odd">
    <td><p>[ibmuser@student ~]$ /home/ibmuser/WAS-Automation-LabFiles/wsalabs/lab2-MemoryLeak/klp-curl_MLApp_add_was.sh</p>
    <p>--> forcing garbage collection</p>
    <p>--> Wait 10 seconds for memory to be released</p>
    <p>--> Invoking memory leak app</p>
...
12. Stop the WebSpPhere Application Server
